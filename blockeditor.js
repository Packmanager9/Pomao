...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    function setUp(canvas_pass, style = "#000000") {
        ...
        window.addEventListener('pointerdown', e => {
            ...
            TIP_engine.x = ((XS_engine-xdisp)*scale)
            TIP_engine.y = ((YS_engine-ydisp)*scale)
            TIP_engine.body = TIP_engine
            blockpoint = new Point(TIP_engine.x, TIP_engine.y)
            // example usage: if(object.isPointInside(TIP_engine)){ take action }
            window.addEventListener('pointermove', continued_stimuli);
        });
        window.addEventListener('pointerup', e => {
            window.removeEventListener("pointermove", continued_stimuli);
            if(TIP_engine.x > blockpoint.x){
                let rect = new Rectangle(blockpoint.x, blockpoint.y, Math.abs(TIP_engine.x-blockpoint.x), Math.abs(TIP_engine.y-blockpoint.y), "red")
                blocks.push(rect)
            }else{
            let rect = new Rectangle(blockpoint.x-Math.abs(TIP_engine.x-blockpoint.x), blockpoint.y-Math.abs(TIP_engine.x-blockpoint.x), Math.round(blockpoint.x), Math.round(blockpoint.y), "red")
            blocks.push(rect)
            }
        })
        function continued_stimuli(e) {
            ...
            TIP_engine.body = TIP_engine
            if(TIP_engine.x > blockpoint.x){
                let rect = new Rectangle(blockpoint.x, blockpoint.y, Math.abs(TIP_engine.x-blockpoint.x), Math.abs(TIP_engine.y-blockpoint.y), "#FF000044")
                rect.draw()
            }else{
            let rect = new Rectangle(blockpoint.x-Math.abs(TIP_engine.x-blockpoint.x), blockpoint.y-Math.abs(TIP_engine.x-blockpoint.x), Math.round(blockpoint.x), Math.round(blockpoint.y),  "#FF000044")
            rect.draw()
            }
        }
    }
...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    let zero = new Circle(0,0,10,"white")

    function main() {
        canvas_context.clearRect(-10000, -10000, canvas.width*(scale)*100, canvas.height*(scale)*100)  // refreshes the image
